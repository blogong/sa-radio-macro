...
@Component
class WebClientHelper(private val httpHeaderProperties: net.bk24.macro.core.infra.clients.HttpHeaderProperties) {
    ...
    companion object {
        fun createFormData(authCode: String): BodyInserters.FormInserter<String> {
            return BodyInserters.fromFormData("auth_code", authCode)
                ...
                .with("content", net.bk24.macro.core.infra.clients.WebClientHelper.Companion.getRandomKoreanString())
                .with("season_level_join_flag", "Y")
                .with("season_level_sn", "1")
                .with("user_level", "47")
        }

        private fun getRandomKoreanString(length: Int = 9): String {
            val start = 0xAC00
            val end = 0xD7A3
            return (1..length)
